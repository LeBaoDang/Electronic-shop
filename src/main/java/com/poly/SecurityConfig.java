...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
		...
		auth.userDetailsService(username -> {
			try {
				...
				String[] roles = user.getAuthorities().stream() /* Lấy các vai trò của người dùng -> đổi thành mảng */
						.map(er -> er.getRole().getId()).collect(Collectors.toList())
						.toArray(new String[0]); /* Đổi thành mảng */
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.formLogin().loginPage("/security/login/form") /* địa chỉ form đăng nhập */
